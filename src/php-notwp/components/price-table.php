<table class="price-table">
          <tbody>
            <tr class="price-table__row" is-first-row="true">
              <td class="price-table__cell">
                <div class="caption">
                  <img src="../img/content/ways/country/flag-china.png" alt="флаг">
                  <span>Тайланд</span>
                </div>
              </td>
              <td class="price-table__cell">
                <span>Паттайя</span>
              </td>
              <td class="price-table__cell">
                <span>Вылет 09 апреля</span>
              </td>
              <td class="price-table__cell">
                <span> на 7 ночей</span>
              </td>
              <td class="price-table__cell price">
                <span> от 28 000 руб.</span>
              </td>
              <td class="price-table__cell button">
                <button class="price-table__btn btn btn-price">хочу поехать</button>
              </td>
            </tr>
            <tr class="price-table__row" is-first-row="false">
              <td class="price-table__cell">
              <div class="caption d-none d-block-md">
                  <img src="../img/content/ways/country/flag-china.png" alt="флаг">
                  <span>Тайланд</span>
                </div>
              </td>
              <td class="price-table__cell">
                <span>Паттайя</span>
              </td>
              <td class="price-table__cell">
                <span>Вылет 09 апреля</span>
              </td>
              <td class="price-table__cell">
                <span> на 7 ночей</span>
              </td>
              <td class="price-table__cell price">
                <span> от 28 000 руб.</span>
              </td>
              <td class="price-table__cell button">
                <button class="price-table__btn btn btn-price">хочу поехать</button>
              </td>
            </tr>
            <tr class="price-table__row" is-first-row="true">
              <td class="price-table__cell">
              <div class="caption">
                  <img src="../img/content/ways/country/flag-china.png" alt="флаг">
                  <span>Тайланд</span>
                </div>
              </td>
              <td class="price-table__cell">
                <span>Паттайя</span>
              </td>
              <td class="price-table__cell">
                <span>Вылет 09 апреля</span>
              </td>
              <td class="price-table__cell">
                <span> на 7 ночей</span>
              </td>
              <td class="price-table__cell price">
                <span> от 28 000 руб.</span>
              </td>
              <td class="price-table__cell button">
                <button class="price-table__btn btn btn-price">хочу поехать</button>
              </td>
            </tr>
            <tr class="price-table__row" is-first-row="true">
              <td class="price-table__cell">
              <div class="caption">
                  <img src="../img/content/ways/country/flag-china.png" alt="флаг">
                  <span>Тайланд</span>
                </div>
              </td>
              <td class="price-table__cell">
                <span>Паттайя</span>
              </td>
              <td class="price-table__cell">
                <span>Вылет 09 апреля</span>
              </td>
              <td class="price-table__cell">
                <span> на 7 ночей</span>
              </td>
              <td class="price-table__cell price">
                <span> от 28 000 руб.</span>
              </td>
              <td class="price-table__cell button">
                <button class="price-table__btn btn btn-price">хочу поехать</button>
              </td>
            </tr>
            <tr class="price-table__row" is-first-row="true">
              <td class="price-table__cell">
              <div class="caption">
                  <img src="../img/content/ways/country/flag-china.png" alt="флаг">
                  <span>Тайланд</span>
                </div>
              </td>
              <td class="price-table__cell">
                <span>Паттайя</span>
              </td>
              <td class="price-table__cell">
                <span>Вылет 09 апреля</span>
              </td>
              <td class="price-table__cell">
                <span> на 7 ночей</span>
              </td>
              <td class="price-table__cell price">
                <span> от 28 000 руб.</span>
              </td>
              <td class="price-table__cell button">
                <button class="price-table__btn btn btn-price">хочу поехать</button>
              </td>
            </tr>
          </tbody>
          <tfoot>
            <tr class="price-table__footer">
              <td colspan="6">
                <small class="price-table__footer-small">*цена указана на человека при условии размещения в двухместном номере
                  и действительна на данный момент.</small>
                <button class="price-table__footer-btn">
                  нет вашего направления?
                </button>
              </td>
            </tr>
          </tfoot>
        </table>
      </div>
